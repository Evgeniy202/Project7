...
@section('content')
    ...
    <div class="container">
        ...
        <div class="tab-content" id="nav-tabContent">
            <div class="tab-pane fade show active" role="tabpanel" aria-labelledby="list-orders-list">
            ...
            <div class="row">
                ...
                <hr class="mt-3">
                <strong class="col-md-2">Номер телефону:</strong>
            ...
            <table class="table text-dark">
                ...
                @foreach($products as $product)
                    <tr>
                        <th scope="row"><a
                                href="{{ route('redirect-to-product', $product->id) }}">{{ $product->title }}</a></th>
                        <td><a href="{{ route('redirect-to-product', $product->id) }}"><img
                                    src="{{ asset('/storage/'.$product->image) }}" class="img-fluid rounded"
                                    width="50px"></a></td>
                        <td>${{ $product->price }}</td>
                        <td>
                            <form action="{{ route('change-number', $product->order_prod_id) }}" method="POST">
                                @csrf
        ...
        <button id="show_form" class="btn btn-outline-success col-12">Додати новий товар</button>
        <form id="add_form" method="post" action="{{ route('add-product', $order->id) }}" hidden>
            ...
            <div class="mt-2">
                <input name="search" id="search" class="mr-sm-2 bg-light form-control" type="text"
                       placeholder="Пошук за номером замовлення" aria-label="Search" required>
            </div>
            <div class="mt-2">
