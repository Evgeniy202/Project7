@extends('layouts.app')
@section('title', 'Project - Вибрані продукти')
@section('content')
    <main class="container">
        <header class="d-sm-flex align-items-center border-bottom mb-4 pb-3">
            @if(!empty($products))
                @if(count($products) < 2)
                    <strong class="d-block py-2">{{ count($products) }} продукт</strong>
                @elseif(count($products) > 1)
                    <strong class="d-block py-2">{{ count($products) }} продукти</strong>
                @endif
            @else
                <strong class="d-block py-2">0 продуктів</strong>
            @endif
        </header>
        <!-- ========= вміст елементів ========= -->
        @if(empty($products))
            <h3 class="text-center">Порожньо</h3>
        @else
                            ...
                            <div class="info-aside">
                                <div class="price-wrap pt-1">
                                    @if (!empty($discounts[$product->id]))
                                        <strong class="price">${{ $product->price - $discounts[$product->id] }} зі знижкою!</strong>
                                        <br>
                                ...
                                <div class="mb-3">
                                    ...
                                    <a href="{{ route('remove-selected', $product->id) }}"
                                       class="btn btn-outline-danger col-8 m-1"> Видалити з вибраних </a>
                                </div>
                            </div> <!-- info-aside.// -->
                        </aside> <!-- col.// -->
                    </div> <!-- row.// -->
